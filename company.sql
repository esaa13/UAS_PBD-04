...
--
-- Host: 127.0.0.1
-- Waktu pembuatan: 30 Jul 2025 pada 10.47
...
-- Prosedur
--
CREATE DEFINER=`root`@`localhost` PROCEDURE `jumlah_customer` ()   BEGIN
SELECT COUNT(*) AS total_customer FROM customers;
END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `list_order_by_customer` (IN `cust_id` INT, OUT `jumlah` INT)   BEGIN
	DECLARE done INT DEFAULT FALSE;
    DECLARE ord_num INT;
    DECLARE cur CURSOR FOR SELECT orderNumber FROM orders WHERE customerNumber = cust_id;
    DECLARE CONTINUE HANDLER FOR NOT FOUND SET done = TRUE;
    SET jumlah = 0;
    OPEN cur;
    read_loop:LOOP
    FETCH cur INTO ord_num;
...
-- Fungsi
--
CREATE DEFINER=`root`@`localhost` FUNCTION `total_offices` () RETURNS INT(11) DETERMINISTIC BEGIN
DECLARE total INT;
SELECT COUNT(*) INTO total FROM offices;
RETURN total;
END$$

CREATE DEFINER=`root`@`localhost` FUNCTION `total_order_customer` (`cust_id` INT, `status_input` VARCHAR(20)) RETURNS INT(11) DETERMINISTIC BEGIN
	DECLARE total int;
    SELECT COUNT(*) INTO total FROM orders
    WHERE customerNumber = cust_id AND status = status_input;
    RETURN total;
END$$
...
(497, 'Susulja', 'Kelompok', 'PBD', '089504456062', 'Jl. Godean, Yogyakarta', NULL, 'Yogyakarta', NULL, NULL, 'Japan', 1002, 0.00),
(498, 'Soesoeldja', 'Pebede', 'Basis Data', '089504412300', 'Jl. Ring Road Utara', NULL, 'Yogyakarta', NULL, NULL, 'Indonesia', 1002, 75000.00),
(499, 'Japeli', 'Kelompok', 'PBD', '085312300000', 'Jl. Palagan No. 4', NULL, 'New York', NULL, NULL, 'USA', 1002, 80000.00);

--
-- Trigger `customers`
--
DELIMITER $$
CREATE TRIGGER `trg_after_insert_customer` AFTER INSERT ON `customers` FOR EACH ROW BEGIN
	INSERT INTO log_customers(customer_id)
...
CREATE TRIGGER `trg_before_delete_customer` BEFORE DELETE ON `customers` FOR EACH ROW BEGIN
	DECLARE jumlah INT;
    SELECT COUNT(*) INTO jumlah FROM orders WHERE customerNumber = OLD.customerNumber;
    IF jumlah > 0 THEN
    SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'Tidak bisa hapus customer yang punya pesanan';
END IF;
END
$$
DELIMITER ;
DELIMITER $$
CREATE TRIGGER `trg_before_update_credit` BEFORE UPDATE ON `customers` FOR EACH ROW BEGIN
...
--
-- Struktur dari tabel `log_customers`
--
...
--
-- Dumping data untuk tabel `log_customers`
--

INSERT INTO `log_customers` (`id`, `customer_id`, `action_time`) VALUES
(1, 497, '2025-07-30 14:52:33'),
(2, 498, '2025-07-30 15:23:24'),
(3, 499, '2025-07-30 15:28:55');
...
--
-- Stand-in struktur untuk tampilan `view_all_customers`
-- (Lihat di bawah untuk tampilan aktual)
--
CREATE TABLE `view_all_customers` (
...
--
-- Stand-in struktur untuk tampilan `view_name_country`
-- (Lihat di bawah untuk tampilan aktual)
--
CREATE TABLE `view_name_country` (
`customerName` varchar(50)
,`country` varchar(50)
);
...
--
-- Stand-in struktur untuk tampilan `view_usa_customers`
-- (Lihat di bawah untuk tampilan aktual)
--
CREATE TABLE `view_usa_customers` (
`customerNumber` int(11)
,`customerName` varchar(50)
,`contactLastName` varchar(50)
,`contactFirstName` varchar(50)
...
--
-- Struktur untuk view `view_all_customers`
--
DROP TABLE IF EXISTS `view_all_customers`;

CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `view_all_customers`  AS SELECT `customers`.`customerNumber` AS `customerNumber`, `customers`.`customerName` AS `customerName`, `customers`.`contactLastName` AS `contactLastName`, `customers`.`contactFirstName` AS `contactFirstName`, `customers`.`phone` AS `phone`, `customers`.`addressLine1` AS `addressLine1`, `customers`.`addressLine2` AS `addressLine2`, `customers`.`city` AS `city`, `customers`.`state` AS `state`, `customers`.`postalCode` AS `postalCode`, `customers`.`country` AS `country`, `customers`.`salesRepEmployeeNumber` AS `salesRepEmployeeNumber`, `customers`.`creditLimit` AS `creditLimit` FROM `customers` ;

-- --------------------------------------------------------

--
-- Struktur untuk view `view_name_country`
--
DROP TABLE IF EXISTS `view_name_country`;

CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `view_name_country`  AS SELECT `customers`.`customerName` AS `customerName`, `customers`.`country` AS `country` FROM `customers` ;

-- --------------------------------------------------------

--
-- Struktur untuk view `view_usa_customers`
--
DROP TABLE IF EXISTS `view_usa_customers`;

CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `view_usa_customers`  AS SELECT `view_all_customers`.`customerNumber` AS `customerNumber`, `view_all_customers`.`customerName` AS `customerName`, `view_all_customers`.`contactLastName` AS `contactLastName`, `view_all_customers`.`contactFirstName` AS `contactFirstName`, `view_all_customers`.`phone` AS `phone`, `view_all_customers`.`addressLine1` AS `addressLine1`, `view_all_customers`.`addressLine2` AS `addressLine2`, `view_all_customers`.`city` AS `city`, `view_all_customers`.`state` AS `state`, `view_all_customers`.`postalCode` AS `postalCode`, `view_all_customers`.`country` AS `country`, `view_all_customers`.`salesRepEmployeeNumber` AS `salesRepEmployeeNumber`, `view_all_customers`.`creditLimit` AS `creditLimit` FROM `view_all_customers` WHERE `view_all_customers`.`country` = 'USA'WITH CASCADED CHECK OPTION  ;
...
ALTER TABLE `customers`
  ...
  ADD KEY `idx_name_country` (`customerName`,`country`);
...
--
-- Indeks untuk tabel `log_customers`
--
ALTER TABLE `log_customers`
  ADD PRIMARY KEY (`id`);
...
ALTER TABLE `log_login`
  ADD PRIMARY KEY (`id`),
  ADD KEY `idx_emp_time` (`employee_id`,`login_time`);
...
ALTER TABLE `products`
  ...
  ADD KEY `idx_product_price` (`buyPrice`,`MSRP`);
...
--
-- AUTO_INCREMENT untuk tabel `log_customers`
--
ALTER TABLE `log_customers`
...
--
ALTER TABLE `product_details`
  ADD CONSTRAINT `product_details_ibfk_1` FOREIGN KEY (`productCode`) REFERENCES `products` (`productCode`);
COMMIT;
